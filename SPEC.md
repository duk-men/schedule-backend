...
| 휴게 시각 | ~~CP-SAT 결정변수~~(버그 2 대응, 4절 참고)에서 고정 규칙으로 되돌림 (10절): 오픈(8/9시)은 12:30부터, 마감(12/13시)은 15:30부터, 출근 순서대로 한 명씩 이어서(예: 12:30~13:30 → 13:30~14:30). 미들·쩜오·야간은 휴게 없음. 사용자가 명시적으로 요청한 고정 규칙이라 버그 2가 재발할 여지(바 인원 미달)를 감수함 — 실제로 인력이 빠듯한 주는 완화 페널티(2.5·12.5절)가 더 자주 걸리는 것으로 관찰됨 |
